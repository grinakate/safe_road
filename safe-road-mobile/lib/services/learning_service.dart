...
class LearningService {
  ...
  Future<List<Question>> getTestQuestions(String sessionId) async {
    final response = await _apiClient.get('/learning/test/session/$sessionId/questions');
    ...
    if (response.statusCode == 200) {
      ...
      return data.map<Question>((json) {
        ...
        final text = (json['questionText'] ?? '') as String;
        final optionsRaw = (json['options'] as List<dynamic>? ) ?? [];
        final options = optionsRaw.map<AnswerOption>((opt) {
          final number = (opt['number'] as num).toInt();
          final optionText = (opt['text'] ?? '') as String;
          return AnswerOption(id: number, text: optionText, feedback: '', isCorrect: false);
        }).toList();
        final correctNumber = (json['correctAnswerNumber'] != null) ? (json['correctAnswerNumber'] as num).toInt() : -1;
        return Question(id: id, text: text, options: options, correctAnswerId: correctNumber);
      }).toList();
    } else {
      throw Exception('Failed to load session questions: ${response.statusCode}');
    }
  }

  /// Submit single answer for the active session; returns server feedback
  Future<SubmitAnswerResult> submitSessionAnswer(String sessionId, int questionId, int selectedAnswerId) async {
    // Backend expects SubmitAnswerRequest { answerId }
    final body = {'answerId': selectedAnswerId};
    final response = await _apiClient.post('/learning/test/session/$sessionId/answer', body);

    if (response.statusCode == 200) {
      return SubmitAnswerResult.fromJson(jsonDecode(response.body));
    } else {
      throw Exception('Failed to submit session answer: ${response.statusCode}');
    }
  }

  /// Submit all answers for the session in one call. Backend expects map questionId->answerId
  Future<TestSessionSubmitResult> submitSessionAnswers(String sessionId, Map<int, int> answers) async {
    // Convert keys to strings to ensure JSON maps serialize properly
    final body = {'answers': Map.fromEntries(answers.entries.map((e) => MapEntry(e.key.toString(), e.value)))};
    // However backend expects the raw map, not nested under 'answers', so send body as the map itself
    final response = await _apiClient.post('/learning/test/$sessionId/submit-answer', answers);

    if (response.statusCode == 200) {
      return TestSessionSubmitResult.fromJson(jsonDecode(response.body));
    } else {
      throw Exception('Failed to submit session answers: ${response.statusCode}');
    }
  }
